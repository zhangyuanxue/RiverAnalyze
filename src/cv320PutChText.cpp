...
diaphaneity:透明度
**********************************/
void cv320PutChText::getFont(int *type, CvScalar *size, bool *underline, float *diaphaneity)
{
...
diaphaneity:透明度
**********************************/
void cv320PutChText::setFont(int *type, CvScalar *size, bool *underline, float *diaphaneity)
{
	// 参数合法性检查 
	if (type)
	{
...
**********************************/
int cv320PutChText::putText(cv::Mat &frame, const char* text, CvPoint pos,const int frontSize)
{
...
返回值：返回成功输出的字符长度，失败返回-1
**********************************/
int cv320PutChText::putText(cv::Mat &frame, std::string& text, CvPoint pos, CvScalar color,const int frontSize)
{
    return putText(frame, stows(text).c_str(), pos, color, frontSize);
}


int cv320PutChText::putText(cv::Mat &frame, const char *text, CvPoint pos, CvScalar color,const int frontSize)
 {
	/*
...
int cv320PutChText::putText(cv::Mat &frame, const wchar_t *text, CvPoint pos, CvScalar color,const int frontSize)
{
	int i = 0;
	restoreFont(frontSize);
	if (frame.empty()) return -1;
...
void cv320PutChText::putWChar(cv::Mat &frame, wchar_t wc, CvPoint &pos, CvScalar color)
{
	// 根据unicode生成字体的二值位图  
	IplImage img = frame;
